    ...
    jsPsych.text = (function() {
        ...
        plugin.create = function(params) {
            ...
            for (var i = 0; i < trials.length; i++) {
				...
				trials[i].subid = params.subid || 0;	//Add a subject ID box
            }
            return trials;
        };
        ...
        plugin.trial = function(display_element, block, trial, part) {
			...
			var test_input = '<form action="demo_form.asp"> \
				  First Name: <input type="text" name="FirstName" value="" id = "fNameTag"><br> \
				  Last Name: <input type="text" name="LastName" value="" id = "lNameTag"><br> \
				 </form>'
			...
			if (trial.subid === 0){
				...
				display_element.html(test_input);
				display_element.append(trial.text);
			}
            ...
            var key_listener = function(e) {
                if (e.which == trial.cont_key && cont_key_down) {
					if (trial.subid !== 0){
						name = document.getElementById('fNameTag').value + document.getElementById('lNameTag').value;
						//document.getElementById("instructions").innerHTML = name;
					}
                    save_data();
